...
function appendXMLNotifications(buffer, notifications, nodeName, publishing) {
	if (notifications && notifications.name && notifications.name.length > 0) {
		...
		notifications.name.forEach((item, index) => {
			...
			const roleTypeName = publishing ? 'depot' : 'asset library';
...
function appendXMLTaskTimers(buffer, taskTimers, publishing) {
...
function serializeDefinition(
	...
	nodes?.forEach((item) => {
		...
		appendXMLTaskTimers(buffer, item.data.taskTimers, publishing);
